...
/*
 *	This file describes the interface that RALpage requires to the graphics system at Version 1.4.
 *	
 *	''Hardware'' in this context refers to a pointer to windows and/or bitmaps and is the lowest level
 *	of access that RALpage is interested in. Any Hardware parameter may be expected to be NULL.
 */
...
/*
 *	InitHardware () returns a default device which RALpage may use immediately (or NULL if not appropriate).
 *	Its size and shape are not defined. Most typically the user will want to start up another device
 *	before it is used anyway. No attempt will be made by RALpage to Destroy the resulting
 *	device.
 */
...
/*
 *	ScreenSize () allows RALpage to determine how large an array of sample points to expect.
 *	It should return the length of the side of the sample square.
 *	
 *	BuildScreen () returns a set of sampling coordinates to RALpage to hand to the users spot-function
 *	
 *	SetScreen () allows RALpage to set the thresholds for each sample point so that half-tone bitmaps
 *	can be made.
 */
 ...
 *	of the flush operator). Typically is is a no-op.
 */
