...
struct hardware *InitHardware ()
 {
#ifdef RUNLENGTH
	InstallOp ("runlength",		RunLength,		1, 0, 0, 0, File);
#endif	
	wwstartup (2);
...
struct hardware *NewWindowHardware (width, height) int width, height;
 {
 	...
 	if ((w = wwxget (boxbuild (0, 0, width, height), 2, "RALpage", 0)) == NULL)
 		return NULL;
...
void BitBltLine (h, fromPoint, toPoint, rop) 
	...
	int rop;
 {
	...
	switch (single_rop [rop])
	 {
	 	case ROP_FALSE: 	rop = WWAND | WWNOT; 	break;
...
void BitBltBlob (to, top, height, left, right, rop) struct hardware *to; int top, height, *left, *right, rop;
 {
 	...
 	for (i = top; i < height; i++)
 		bmbox (boxbuild (left[i - offset], i, right[i - offset] - 1, i), op);
...
static int RunLength (f) Object f;
 {
 	FILE *fp = BodyFile (f)->f.f_ptr;
 	bitmap *bm = DeviceBitmap (gstate->device->dev);
 	struct pixrect *pr = (struct pixrect *) unportask (bm, ASKBMMEMORY);
 	struct mpr_data *data = mpr_d (pr);
 	char *bits = (char *) (data->md_image);
 	/* Phew! */
 	
 	run_length_write (fp, bm->bm_box.b_right + 1, bm->bm_box.b_bottom + 1, bits);
