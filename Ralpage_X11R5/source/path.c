...
/* APOLLO COMPILER BUG: cannot cope with conditional function call
 	if (!(*(EmptyPath (gstate->path) ? MoveTo : LineTo)) (gstate->path, ExtToInt (MovePoint (centre, start))))
		return FALSE;
*/
 	if (EmptyPath (gstate->path))
		status = MoveTo (gstate->path, ExtToInt (MovePoint (centre, start)));
	else
		status = LineTo (gstate->path, ExtToInt (MovePoint (centre, start)));
