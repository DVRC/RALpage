...
static int Chdir (dir) Object dir;
 {
 	...
 	VOID strncpy (buffer, (char *) BodyString (dir), l);
 	buffer [l] = '\0';
...
static int Execv (args, name) Object args, name;
 {
 	...
 	for (i = 0; i < l; i++)
 	 {
 		...
 		if (TypeOf (elem) != String)
 		...
 		else
 		 {
 			...
 			VOID strncpy (av[i], (char *) BodyString (elem), l);
 			av[i][l] = '\0';
 		 }
 	 }
 	av[l] = NULL;
 	...
 	VOID strncpy (buffer, (char *) BodyString (name), nl);
 	buffer [nl] = '\0';
...
static int System (s) Object s;
 {
 	...
 	VOID strncpy (buffer, (char *) BodyString (s), l);
 	buffer [l] = '\0';
...
/* PGetEnv by jgm with mods by caag */
static int PGetEnv (string) Object string;
 {
	char *s, *getenv (), buffer [BUFSIZ];
	
	Bcopy ((unsigned char *) buffer, BodyString (string), lengthString (string));
	buffer [lengthString (string)] = '\0';

	if ((s = getenv (buffer)) != NULL)
		VOID Push (OpStack, MakeString ((unsigned char *)s, strlen (s)));
