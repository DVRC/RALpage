...
/* datri 6-1-90 added extern def for InitHardware */
extern struct hardware *InitHardware ();
...
static int GrabBits (x, y, w, h) Object x, y, w, h;
 {
 	...
 	Push (OpStack, MakeString ((unsigned char *) s, size));
 	Free (s);
...
struct device *UniqueDevice (d) struct device *d;
 {
 	...
 	BitBlt ((struct hardware *) NULL, res->dev, NewDevicePoint (0, 0), NewDevicePoint (0, 0), ex, ROP_FALSE);
