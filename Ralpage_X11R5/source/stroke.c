...
static void Dash ()
 ...
 /* APOLLO COMPILER BUG: cannot cope with conditional function pointers
...
float LineSegment (p, new, ehere, enow, width, last_angle, last_type) Path p, new; Point ehere, enow; float width, last_angle; enum pelem_type last_type;
 {
 	float angle, length = sqrt ((enow.y - ehere.y) * (enow.y - ehere.y) + (enow.x - ehere.x) * (enow.x - ehere.x));
	Matrix old;
	
	if (enow.y - ehere.y == 0 && enow.x - ehere.x == 0)
		angle = 0;
	else
		angle = atan2 (enow.y - ehere.y, enow.x - ehere.x);
	old = gstate->CTM;
