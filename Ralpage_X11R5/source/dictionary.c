...
Type type_table [TYPES], *next_type = type_table;
static Type MakeDictBody ();
...
Type MakeType (size) int size;
 {
	Type res = MakeDictBody (size);
	
	res->type_index = next_type - type_table;
	*next_type++ = res;
...
Object MakeDict (size) int size;
 {
	...
	res.u.Dictionary = MakeDictBody (size);
	return res;
 }
...
#define Body(item) ((item).u.Dictionary)
...
int Equal (a, b) Object a, b;
 {
	return (a.u.Integer == b.u.Integer || TypeOf (a) == Array && a.Length == 0) &&
		TypeOf (a) == TypeOf (b) &&
		a.Length == b.Length;
 }

#define Equal(a, b) (((a).u.Integer == (b).u.Integer || TypeOf (a) == Array && (a).Length == 0) && \
		TypeOf (a) == TypeOf (b) && \
		(a).Length == (b).Length)
...
HashFromKey (key, size) Object key; int size;
 {
	return (key.Length + BodyInteger (key) + (unsigned) TypeOf (key)) % size;
 }

#define HashFromKey(key, size) ((((key).Length + (key).u.Integer + (unsigned) TypeOf (key)) >> 2) % (size))
...
#else
 		res = DictFind (Body (DictStack->stack_body[i])->dict_body,
 					key,
 					Body (DictStack->stack_body[i])->dict_size);
#endif
