...
/*
 * This file implements the input/output calls in the PostScript language.
 *
 * A PostScript language file corresponds reasonably closely to a Unix file, so this is not
 * too hard.
 *
 ...
 * but the input process continues until the interpreter sees a complete expression, with
 ...
 *		interpreter to just read from them.
...
InitFile ()
 {
	...
	InstallOp ("filecheck",		PFileCheck,	2, 1, 0, 0, String, String);
	...
	if (lengthString (string1) == 0
 	...
 	else
 	 {
 		...
 		modes [0] = mode; modes [1] = '\0';
 	 	VOID Bcopy ((unsigned char *) name, BodyString (string1), lengthString (string1));
...
static int PFileCheck (string1, string2) Object string1, string2;
 {
 	int mode;
...
static int EditLine ()
 {
	...
	Cbreak (TRUE);
	return Push (OpStack,
			ReadOnly (FileString (MakeString (Bcopy ((unsigned char *) Malloc ((unsigned) length), buf, length),
							  length))));
 }
...
static int EditStatement ()
 {
 	...
 	Cbreak (TRUE);
 	return Push (OpStack,
 			ReadOnly (FileString (MakeString ((unsigned char *) Bcopy (Malloc ((unsigned) length), (unsigned char *) buf, length),
 							  length))));
 }
