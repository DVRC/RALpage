...
static int LoadDiskCache ();
int LoadXCache (), LoadVfontCache ();
...
InitCache ()
 {
	...
	InstallOp ("loaddiskcache",	LoadDiskCache,		1, 0, 0, 0, String);
	InstallOp ("loadxcache",	LoadXCache,		1, 1, 0, 0, String);
	InstallOp ("loadvfontcache",	LoadVfontCache,		1, 1, 0, 0, String);
 }
...
CharStore (key, width, dev, swidth, sheight) Object key; Point width; struct device *dev; int swidth, sheight;
 {
 	struct char_table *p;
	...
	if (p = CharAddition (CharTable, gstate->show->ccache, key, cmax))
	 {
		p->char_width = width;
		p->char_device = dev;
		p->char_swidth = swidth;
		p->char_sheight = sheight;
		++csize;
	 }
	else
...
int SetCacheDevice (name, char_width, llx, lly, urx, ury)
		Object name; Point char_width; float llx, lly, urx, ury;
 {
	...
	swidth = origin.hx - left; sheight = origin.hy - bottom;
 	...
 	new_char = NewCacheDevice (ccache->mat, (int) (right - left), (int) (top - bottom), swidth, sheight);
 	CharStore (name, char_width, LinkDevice (new_char), swidth, sheight);
 	SetDevice (new_char);
 	
 	PathFree (gstate->clip);
  	gstate->clip = NewClipPath (0.0, width, height, 0.0);
	gstate->CTM.tx = origin.hx - left;
 	gstate->CTM.ty = origin.hy - bottom;
...
int FindCache ()
 {
	...
	DeviceBBox (&left, &right, &top, &bottom,
 	...
 	if (msize == mmax)
 	 {
 	 	Message ("removing a cache to make room");
 		DestroyCache (CacheRemove (cache_ring->cache_prev));
	 }
	origin = ExtToInt (NewPoint (0.0, 0.0));
	swidth = origin.hx - left; sheight = origin.hy - bottom;
...
int CacheShow (name, cp) Object name; HardPoint cp;
 {
 	...
 	from.dx = from.dy = 0;
 	to.dx = cp.hx - b->char_swidth + 0.5;
 	to.dy = cp.hy - b->char_sheight + 0.5;
	
	Paint (b->char_device->dev, gstate->device->dev, from, to, HardwareExtent (b->char_device->dev), gstate->colour);
	return TRUE;
 }
...
static int LoadDiskCache (string) Object string;
 {
	Matrix m; float bbox[4];
 	...
 	VOID sprintf (disk_name, "%.*s", lengthString (string), BodyString (string));
 	Message (disk_name);
 	...
 	while (!feof (fp))
 	 {
		...
		float width, height;
 	 	...
 	 	if (fscanf (fp, "%s %f %f %d\n",
 	 			char_name,
 	 			&width, &height,
 	 			&length) == EOF)
 	 		break;
	  	VOID fread (bits, length, 1, fp);
 	 	VOID getc (fp);
 	 ...
 	 if (CharTable[i].char_cache == ccache)
 	  {
	 	...
	 	VOID fprintf (fp,
 	 		...
 	 		CharTable [i].char_width.x,
 	 		CharTable [i].char_width.y,
 	 		length);
 	 	VOID fwrite (bits, length, 1, fp);
 	 	putc ('\n', fp);
/*  	 	Free (bits);*/
