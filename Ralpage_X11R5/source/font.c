...
Object AWidthShowName, GRestoreName, LoadDiskCacheName;

static last_fid = 0;

static int Update ();
static int XMoveShow ();
...
InitFont ()
 {
 	...
 	AWidthShowName	= Cvx (NameFrom ("awidthshow"));
 	GRestoreName	= Cvx (NameFrom ("grestore"));
 	LoadDiskCacheName = Cvx (NameFrom ("loaddiskcache"));
	...
	InstallOp ("update",		Update,		1, 0, 0, 0, Boolean);
 	InstallOp ("xmoveshow",		XMoveShow,	2, 0, 0, 0, Float, String);
	
 	Install ("FontDirectory",	FD = MakeDict (200));
 	
 	zeroPoint = NewPoint (0.0, 0.0);
 	zeroHardPoint = NewHardPoint (0.0, 0.0);
 }

static int XMoveShow (x, s) Object x, s;
 {
 	Point ucp;
 	
	ucp = IntToExt (gstate->cp);
 	ucp.x = BodyReal (x);
 	MoveTo (gstate->path, ExtToInt (ucp));
...
static int PSetFont (font) Object font;
 {
 	...
 	gstate->font = font;
 	SetupFontCache (font);
/* 	SetupFontCache (gstate->font = font);	PYRAMID COMPILER BUG */
 	return TRUE;
 }

void SetupFontCache (font) Object font;
 {
	ExtractMatrix (&fontmatrixCache, DictLoad (font, FontMatrix));
 	encodingCache = BodyArray (DictLoad (font, Encoding));
 	lengthCache = lengthArray (DictLoad (font, Encoding));
...
Vector GetMetrics (ob) Object ob;
 {
...
/* 	met.vx -= WIDTH_BODGE; hershey bodge - they look better closer */
	/*
	...
	*/
	...
	VOID SetCacheDevice (nm, NewPoint (met.vx, met.vy),
 	...
 	for (i = 0; i < l; i++, s++, s++)
 	 {
 	 	...
 	 	po = NewPoint ((float) ((int) ((*s & ~MOVETO) - 64)),
 	 		(float) (int) ((s[1] & ~MOVETO) - 73));
		
		/* APOLLO FUNCTION APPLICATION BUG:
...
/*	 	VOID Push (ExecStack, AWidthShowName);
		...
		VOID Push (OpStack, MakeInteger (space));
		VOID Push (OpStack, MakeReal (space_shim.hx));
		VOID Push (OpStack, MakeReal (space_shim.hy));
	...
	VOID Push (OpStack, MakeInteger ((int) *s));
