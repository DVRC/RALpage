...
static struct name_struct *FindTreeName (s, length, root) unsigned char *s; int length; struct name_struct **root;
 {
	if (*root)
	 {
	 	int cmp = strncmp ((char *) s, (char *) ((*root)->string_body), min (length, (*root)->string_length));
	...
	else
	 {
		...
		r->string_body = Bcopy ((unsigned char *) Malloc ((unsigned) length), (unsigned char *) s, length);
...
static struct name_struct *FindName (s, length) unsigned char *s; int length;
 {
 	...
 	if (p == NULL || p->string_length != length || strncmp ((char *) s, (char *) (p->string_body), length))
 	 {
...
Object NameFrom (s) char *s;
 {
 	Object res;
	
 	res = MakeObject (Name);
 	res.u.Name = FindName ((unsigned char *) s, strlen (s));
...
PrintName (n) Object n;
 {
 	fwrite (BodyName (n), lengthName (n), 1, stdout);
 }
...
Object ParseId (o) Object o;
 {
	...
	number = ParseNumber ((char *) buf, length);
