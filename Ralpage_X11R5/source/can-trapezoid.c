...
/*	fprintf (stderr, "[%d %d] [%d %d] [%d %d] [%d %d] %d %d\n", 
 		lefttop.dx, 	lefttop.dy, 	leftbottom.dx, 	leftbottom.dy,
 		righttop.dx, 	righttop.dy, 	rightbottom.dx, rightbottom.dy, 
 		top, bottom);	*/
 	if (ltx == lbx && rtx == rbx)
 	 {
 	 	if (rtx < ltx)
 	...
 	for (i = top, j = 0; i <= bottom; i++, j++)
 	 {
 		...
 		if (i == rightbottom.dy)
 			rx = rightbottom.dx;
 		else if (i == righttop.dy)
 			rx == righttop.dx;
 		if (i == leftbottom.dy)
 			lx = leftbottom.dx;
 		else if (i == lefttop.dy)
 			lx = lefttop.dx;
 		if (rx < lx)
 		...
 		if (lx == rx)
 			rx++;
 		left [j] = lx; right [j] = rx + 1;
 	 }
 	BitBltBlob (to, top, bottom - top + 1, left, right, single_rop [rop]);
 }
