...
static int Cvi (rep) Object rep;
 {
	...
	v = ParseNumber ((char *) BodyString (rep), lengthString (rep));
	if (TypeOf (v) == Integer)
...
Object MakeString (s, length) unsigned char *s; int length;
 {
	...
	res.u.String = (unsigned char *) (new = Malloc ((unsigned) length));
 	VOID Bcopy ((unsigned char *) new, s, length);
 	res.Length = length;
...
static Object MakeNoCopy (s, length) unsigned char *s; int length;
 {
 	Object res;
...
Object StringFrom (s) char *s;
 {
	Object res;
	
	res = MakeObject (String);
	res.u.String = (unsigned char *) s;
	res.Length = strlen (s);
...
Object getIString (object, begin, length) Object object; int begin, length;
 {
 	return MakeNoCopy (Body (object) + begin, length);
 }
		...
		if (length == BUFSIZE)
 	 	...
 	 	switch (c = Getch (o))
 		 {
			...
			case '(':	++level; *p++ = c; ++length;	    continue;
		 	...
		 	case '\\':
	 			switch (c = Getch (o))
/* no check on file_type for o  klas@nada.kth.se */
 	 				 {
