...
int debugFlag = FALSE, interactive = FALSE, verbose = FALSE;
FILE *vfp = NULL;

main (argc, argv) int argc; char **argv;
 {
 	char rcfile [BUFSIZE], *getenv ();
 	FILE *fp;
 	int here;
 	
	globalArgc = argc;
	globalArgv = argv;
 	library = getenv ("RALPAGELIB");
	if (library == NULL)
 ...
 *	The usual way to do this is to call Error with the name of
 *	the error condition.
 *	
 */
...
Panic (s) char *s;
 {
	fprintf (stderr, "RALpage panic: %s\n", s);
