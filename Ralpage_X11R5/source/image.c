...
static int RealImageMask (width, height, invert, mat, string) Object width, height, invert, mat, string;
 {
	...
	if (origin.vx == bottom.vx && origin.vy == right.vy &&
 	 ...
 	 {		/* simple orientation */
 	 	from = HardwareFromString ((char *) s, w, h);
 	 	if (vrev)
...
DrawBitmap (s, width, height, inv) unsigned char *s; int width, height, inv;
 {
 	...
 	for (h = 0; h < height; h++)
 		for (w = 0; w < width; w++)
 		 	if (((s [h * ((width + 7) >> 3) + (w >> 3)] & (1 << (7 - (w & 0x7)))) != 0) == inv)
  		 	 	FillUnitBox (w, h);
 }
...
struct hardware *UnpaddedHardwareFromString (s, w, h) unsigned char *s; int w, h;
 {
 	...
 	if ((w & 7) == 0)
 		return HardwareFromString ((char *) s, w, h);
 	
 	thin = HardwareFromString ((char *) s, w * h, 1);
 	 	...
 	 	if (dep == 1)
 	 	...
 	 	else
 	 	 {
 	 		struct hardware *mask, *new, *temp;
 	 	 	...
 	 	 	from = HardwareFromString ((char *) s, w * dep, h);
  	 		if (vrev)
 	 			VerticalReverseImage (from);
 	 	 	new = HardwareFromString ((char *) s, w, h);
 	 	 	...
 	 	 	for (i = 0; i < (1<<dep); i++)
 	 	 	 {
	 	 	 	...
	 	 	 	BitBlt ((struct hardware *) NULL, temp, NewDevicePoint (0, 0), NewDevicePoint (0, 0), HardwareExtent (temp), ROP_TRUE);
 	 	 	 	for (j = 0; j < dep; j++)
...
static logtwo[] = { 0, 0, 1, 0, 2, 0, 0, 0, 3 };
