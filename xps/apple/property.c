...
static int PType (item)
     Object item;
{
  return Push (OpStack, Lookup (TypeOf (item), type));
}

static int PCvs (item, string)
     Object item, string;
{
  int l;
  Object t;
...
static int PCvlit (item)
     Object item;
{
  return Push (OpStack, Cvlit (item));
}

static int PCvn (string)
     Object string;
{
  return Push (OpStack, SameFlags (string, Cvn (string)));
}

static int PCvrs (num, base, string)
     Object num, base, string;
{
...
#else
  return Push (OpStack, MakeBool (xCheck (item)));
#endif
}
