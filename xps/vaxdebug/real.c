...
static int Abs (), Add (), Sub (), Mul (), Div (), Neg (), EqEq (), Cvi ();
static int Cvs (), Exp (), Sqrt (), Sin (), Cos (), Atan (), Ln (), Log ();
static int Ceiling (), Floor (), Round (), Truncate ();
...
#define Body(I) ((I).u.Real)

#ifndef Body
static float Body (item)
     Object item;
{
  return item.u.Real;
}
