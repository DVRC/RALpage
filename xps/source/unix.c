...
static int Chdir (), Fork (), Execv (), Wait (), Exit (), System ();
static int Signal (), Kill ();
     ...
     Object dir;
{
  ...
  return Push (OpStack, MakeBool (!chdir (buffer)));
}
...
static int Wait ()
{
  ...
  return Push (OpStack, MakeBool (status & 0200))
...
static int Kill (n, s)
     Object n, s;
{
  return Push (OpStack, MakeBool (0 == kill (BodyInteger (n), BodyInteger (s))));
}
#else
InitUnix ()
{
