 ...
 * from the author.  It is not in the public domain.
 ...
 *
 * Actual control is acheived by stacking continuation operators for
 * the looping operators, declared thus:
 *
 */
 ...
 *
 * Nil is placed at the bottom by each of the looping constructs,
 * which are 'exit'able.
 ...
 * "stop" will unwind the stack down to the Marker, ignoring any Nil
 * or File it might find.
...
InitControl ()
{
  ...
  InstallOp ("if",	PIf,		2, 0, 0, 1, Bool, Array);
  InstallOp ("ifelse",	PIfElse,	3, 0, 0, 1, Bool, Array, Array);
...
static int PIfElse (bool, proc1, proc2)
     Object bool, proc1, proc2;
{
  if (!xCheck (proc1) || !xCheck (proc2))
    return Error (PTypeCheck);
  if (BodyBool (bool))
    VOID Push (ExecStack, proc1);
  else
    VOID Push (ExecStack, proc2);
  return TRUE;
}

static int PRepeat (count, proc)
     Object count, proc;
{
  if (!xCheck (proc))
    return Error (PTypeCheck);
     ...
     Object initial, increment, limit, proc;
{
  ...
  if (integers + reals == 3 && integers != 3 && reals != 3) {
    if (TypeOf (initial) == Integer)   RealInteger(initial);
    if (TypeOf (increment) == Integer) RealInteger(increment);
    if (TypeOf (limit) == Integer)     RealInteger(limit);
  }
...
static int PExit ()
{
  ...
  if ((n = CountTo (Null, ExecStack)) < 0
      || (m = CountTo (Mark, ExecStack)) >= 0 && m < n
      /* don't cross Stopped Context */
      || (p = CountTo (File, ExecStack)) >= 0 && p < n)
    /* don't cross Run context */
