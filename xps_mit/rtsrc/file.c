 ...
 *
 * A PostScript file corresponds reasonably closely to a Unix file, so
 ...
 *
 * %lineedit and %statementedit are files generated by the file
 * operator, rather than merely opened for reading or writing.
 * 
 * %lineedit generates a temporary file containing the text that the
 * user types in on the %stdin stream for one line only.
 * The %stdin is required to be edited while this is going on, which
 * means that it must be in cooked mode (-cbreak). %stdin is normally
 * in cbreak mode. 
 *
 * %statementedit is similar to %lineedit (in that it generates a
 * temporary file), but the input process continues until PostScript
 * sees a complete expression, with "(" matching ")" and "{" matching
 * "}". "[" is not required to match "]", as these are not actually
 * parsed differently from operators (except for their delimiting
 * properties). 
 *
 * There are essentially two ways of implementing these temporary files:
 *
 *	o	We could use real files, and copy user input into
 *		them, allowing the PostScript interpreter to just read
 *		from them. 
 *	o	We could provide some kind of cache using a buffer in
 *		main memory. 
 *
 * The second alternative is likely to reduce certain overheads and,
 * since the code for parsing from strings is needed anyway (token is
 * defined to parse from strings), there is little extra effort in
 ...
 *
 * StringFiles are used internally by token to parse from strings, to
 * avoid duplicating the entire parser. These file types are defined
 * with an enumerated type. 
 *
 * It is defined that closing the %stdin file have no effect other
 * than resetting an end-of-file flag. It is not defined whether the
 * output files are required to be similarly permanent. We assume they
 * are, and introduce a feature of files which is that they may be
 * PERMANENT. This is a flag. It is also not clear if Readable and
 * Writeable access to files is an Access Restriction, as such. We
 * assume so.
 *
 */

extern int errno;
extern void HardUpdate ();
...
static int PFile (), PCloseFile (), PRead (), PReadHexString (), PReadLine ();
static int PReadString (), PBytesAvailable (), PWrite (), PWriteHexString ();
static int PWriteString (), PFlush ();
static int PFlushFile (), PStatus (), PCurFile (), PPrint (), PEcho ();

static Echo ();
static int PseudoFile (), EditLine (), EditStatement ();
...
InitFile ()
{
  ...
  InstallOp ("echo",		PEcho,		1, 0, 0, 0, Bool);
...
#define Body(I) ((I).u.File)

#ifndef Body
static struct file_struct *Body (item)
     Object item;
{
  return item.u.File;
}
...
#ifdef notdef
struct file_struct *BodyFile (item)
     Object item;
{
  return item.u.File;
}
...
/* DCJ: This function not needed with #define right below.
static int Getchar ()
 {
 	return Getc (stdin);
 }
...
static int PFile (string1, string2)
     Object string1, string2;
{
  ...
  if (lengthString (string1) == 0
    ...
    unsigned char name[MAXFILENAME+1], modes[2];
    FILE *fp;
    ...
    if (fp = Fopen (name, modes))
      if (mode == 'r')
	return Push (OpStack, ReadOnly (FileFrom (fp)));
      else
	return Push (OpStack, WriteOnly (FileFrom (fp)));

    return Error (PUnFilename);
  }
}

static int PseudoFile (string1, mode)
     Object string1;
     int mode;
{
  if (EqString (string1, StdIn))
    return mode == 'r' ? Push (OpStack, Fstdin):Error (PInvFileAccess);
  else if (EqString (string1, Terminal))
    return Push (OpStack, Fterminal);
  else if (EqString (string1, StdOut))
    return mode == 'w' ? Push (OpStack, Fstdout):Error (PInvFileAccess);
  else if (EqString (string1, StdErr))
    return mode == 'w' ? Push (OpStack, Fstderr):Error (PInvFileAccess);
  else if (EqString (string1, StatementEdit))
    return mode == 'r' ? EditStatement () : Error (PInvFileAccess);
  else if (EqString (string1, LineEdit))
    return mode == 'r' ? EditLine () : Error (PInvFileAccess);
  else
    return Error (PUnFilename);
}
...
static int EditLine ()
{
  ...
  Cbreak (TRUE);
  Bcopy(t = (unsigned char *) Malloc ((unsigned) length),buf,length);
  return Push (OpStack,ReadOnly (FileString (MakeString (t,length))));
}
...
static int PReadHexString (file, string)
     Object file, string;
{
  ...
  if (Status (file))
    ...
    VOID Push (OpStack, False);
#else
  VOID Push (OpStack, MakeBool (Status (file)));
#endif
  
  return TRUE;
}

static int PReadLine (file, string)
     Object file, string;
{
  ...
  if (c != EOF)
    ...
    VOID Push (OpStack, False);
#else
  VOID Push (OpStack, MakeBool (c != EOF));
#endif
  
  return TRUE;
}

static int PReadString (file, string)
     Object file, string;
{
...
/*ARGSUSED*/
static int PBytesAvailable (file)
     Object file;
{
  return Push (OpStack, MakeInteger (-1));
}

static int PWrite (file, integer)
     Object file, integer;
{
  if (!Status (file))
...
static int PWriteHexString (file, string)
     Object file, string;
{
  FILE *fp;
...
static int PWriteString (file, string)
     Object file, string;
{
     ...
     Object file;
{
  ...
  if (Status (file))
    ...
    return Push (OpStack, False);
#else
  return Push (OpStack, MakeBool (Status (file)));
#endif
}
