...
Object Absent, SysDict;
extern Object Nil;

Object MakeDict ();

static int LengthDict (), CopyDict (), forDict (), ForDict (), PutDict ();
static int GetDict (), PDict (), PBegin (), PEnd (), PDef (), PStore ();
static int PKnown (), PLoad (), PrCheck (), PwCheck (), PReadOnly ();
static int EqDict (), PWhere (), PMaxLength (), PCurrentDict (); 

int hash_tries = 0, hash_collisions = 0, hash_attempts = 0;
...
InitDictionary ()
{
  ...
  TypeInstallOp (Dictionary, "eq", 	EqDict,		2, 1, 0, 0,
		 Dictionary, Dictionary);
  TypeInstallOp (Dictionary, "put", 	PutDict,	3, 0, 0, 0,
		 Dictionary, Poly, Poly);
  TypeInstallOp (Dictionary, "get", 	GetDict,	2, 1, 0, 0,
		 Dictionary, Poly);
  TypeInstallOp (Dictionary, "length", 	LengthDict,	1, 1, 0, 0,
		 Dictionary);
  TypeInstallOp (Dictionary, "copy", 	CopyDict,	2, 0, 0, 0,
		 Dictionary, Dictionary);
  TypeInstallOp (Dictionary, "forall", 	ForDict,	2, 0, 0, 4,
		 Dictionary, Array);
  TypeInstallOp (Dictionary, "readonly", 	PReadOnly,	1, 1, 0, 0,
		 Dictionary);
  TypeInstallOp (Dictionary, "rcheck", 	PrCheck,	1, 1, 0, 0,
		 Dictionary);
  TypeInstallOp (Dictionary, "wcheck", 	PwCheck,	1, 1, 0, 0,
		 Dictionary);
...
Object MakeDict (size)
     int size;
{
  Object res;
  
  MakeObject (res, Dictionary);
  res.u.Dictionary = MakeType (size);
  return res;
}

Object DictFrom (dict)
     Type dict;
{
  Object res;
...
#else
  return Push (OpStack, MakeBool (Body (a) == Body (b)));
#endif
}

static DictReplace (hash, key, value, size, h)
     struct dict_entry *hash;
     Object key, value;
     int size, h;
{
  ...
  for (i = h;;) {
    if (TypeOf (hash[i].entry_key) == Null)
      return FALSE;
    if (DictEqual (key, hash[i].entry_key)) {
     ...
     int size;
{
  ...
  if (TypeOf (key) == String) key = Cvn (key);

  h = (key.Length + BodyInteger (key) + (int) TypeOf (key)) % size;

  for (i = h;;) {
    if (TypeOf (hash[i].entry_key) == Null)  return Absent;
    if (DictEqual (key, hash[i].entry_key)) {
      ++hash_tries;
      return hash[i].entry_value;
    }
    if (++i == size) i = 0;
    else if (i == h) return Absent;
    ++hash_collisions;
  }
}
     ...
     Object key;
{
  ...
  /* Initialize information based solely on the key */
  if (TypeOf (key) == String) key = Cvn (key);
  hashval = key.Length + BodyInteger(key) + (int) TypeOf(key);

  /* Search through the levels of the dict stack from the top down */
  for (level = Height (DictStack) - 1; level >= 0; level--) {
    dict = Body(DictStack->stack_body[level]);
...
static int PutDict (object, key, value)
...
static int CopyDict (object1, object2)
...
static int PDef (key, value)
     Object key, value;
{
  Object dict,t ;	/* SUN optimiser broken */
  
  dict = Top (DictStack);
  if (!wCheck (dict)) return Error (PInvAccess);
  else if (maxDict (dict) == lengthDict (dict) &&
	   (t = DictLoad (dict, key), TypeOf (t)) == Condition)
    return Error (PDictFull);
  else {
    DictStore (dict, key, value);
    return TRUE;
     ...
     Object dict, key;
{
  ...
  if (TypeOf (t) != Condition)
    ...
    return Push (OpStack, False);
#else
  return Push (OpStack, MakeBool (TypeOf (t) != Condition));
#endif
}

static int PLoad (key)
     Object key;
{
  Object res;
...
static int PWhere (key)
     Object key;	/* key --- dict true */
     /* key --- false */
{
  Object where;
...
static int PMaxLength (dict)
     Object dict;
{
  return Push (OpStack, MakeInteger (maxDict (dict)));
}

static int LengthDict (dict)
     Object dict;
{
