 ...
 * 
 *	Edges are characterised by a line description, with topY,
 *	bottomY, topX, bottomX, startingY
 * 	Build edge lists from path.
 *	Sort edges on top Y value.
 *	find topmost interestingY value, which will be the minimum
 *	topY. This becomes interestingY. 
 *	loop
 *		examine edge list from here to end. Move edges with
 *		topY value less than interestingY into interesting
 *		edge set, computing intersections for each as you go.
 *		This should establish next interestingY value, nextY.
 *		form the pairings for all edges in the interesting
 *		set, according to the fill rule at interestingY.
 *			to do this, while inside, keep a pointer to
 *			the last up-transition edge, and pair it with
 *			the next down-transition edge.
 *			for any edges which have changed pairing, emit
 *			a trapezoid from their startingY to
 *			interestingY, (if they were paired) and set
 *			their startingY to interestingY 
...
static int NextLowest ();

static struct edge *edge;
static struct edge **interesting;
...
InitFill ()
{
  ...
  PanicIf (((edge = (struct edge *) malloc(MAXPATHELEMENTS * sizeof(struct edge))) == NULL), "Out of memory.");
  PanicIf (((interesting = (struct edge **) malloc(MAXPATHELEMENTS * sizeof(struct edge *))) == NULL), "Out of memory.");
}
...
#define UpEdge(COUNT_A,COUNT_B,INC_A,INC_B,RULE_A,RULE_B) \
  (((COUNT_B+INC_B)&RULE_A) && !((COUNT_A)&RULE_A) && \
   ((COUNT_A+INC_A)&RULE_A)||((COUNT_A+INC_A)&RULE_A) && \
   !((COUNT_B)&RULE_B) && ((COUNT_B+INC_B)&RULE_B))

#define DownEdge(COUNT_A,COUNT_B,INC_A,INC_B,RULE_A,RULE_B) \
  (((COUNT_B+INC_B)&RULE_B) && ((COUNT_A)&RULE_A) && \
   !((COUNT_A+INC_A)&RULE_A)||((COUNT_A+INC_A)&RULE_A) && \
   ((COUNT_B)&RULE_B) && !((COUNT_B+INC_B)&RULE_B))
...
static void EmitTrapezoid (left, right, top, bottom)
     struct edge *left, *right; int top, bottom;
{
  struct edge *temp;
...
#else
  res = FillPath(gstate->device->dev, gstate->path, 1);
#endif
...
static void PathTrapezoid (left, right, top, bottom)
     struct edge *left, *right; int top, bottom;
{
  int ll, ul, lr, ur;
  struct edge *temp;
  ...
  ll = Xvalue (left->topX, left->topY, left->bottomX, left->bottomY,
	       bottom);
  ul = Xvalue (left->topX, left->topY, left->bottomX, left->bottomY, top);
  lr = Xvalue (right->topX, right->topY, right->bottomX, right->bottomY,
	       bottom);
  ur = Xvalue (right->topX, right->topY, right->bottomX, right->bottomY,
	       top);
  
  GMoveTo (output_path, NewHardPoint ((float) ll, (float) bottom));
  GLineTo (output_path, NewHardPoint ((float) lr, (float) bottom));
  GLineTo (output_path, NewHardPoint ((float) ur, (float) top));
  GLineTo (output_path, NewHardPoint ((float) ul, (float) top));
...
int InitClip ()
{
  ...
  SetClipHardware (gstate->device->dev, (struct hardware *) NULL);
...
static int Clip ()
{
  ...
  if (res) {
    ...
    /*	   output_colour = gstate->colour;
     * gstate->colour = NewColour (0.0, 0.0, 0.0);
     * res = FillPath(gstate->clipdevice->dev, gstate->clip, 1);
     */
    output_device = gstate->clipdevice->dev;
    output_colour = NewColour (0.0, 0.0, 0.0);
    res = FillIt (gstate->clip, NwRule, gstate->device->default_clip,
		  NwRule, EmitTrapezoid);
  }
  return res;
}
...
static void Trapezoids (rule_a, rule_b, emitfn)
     int rule_a, rule_b;
     void (*emitfn)();
{
...
/* BZS - Open code */
#ifndef ThisBit
ThisBit (left, right, where, emitfn)
...
static void RemoveEdges (interestingY, emitfn)
     int interestingY;
     void (*emitfn)();
{
  register int i, j = 0;
...
#ifndef Xvalue
static int Xvalue (ax, ay, bx, by, cy)
     int ax, ay, bx, by, cy;
{
...
static int intercmp (aa, bb)
     struct edge **aa, **bb;
{
  int sign;
  
  sign = (*aa)->value - (*bb)->value;
  if (sign == 0) return (*aa)->bottomX - (*bb)->bottomX;
  return sign;
}
...
static void AddInteresting ()
{
  register int i;
  register struct edge **intp;
  register int topY,bottomY,topX,bottomX;

  nextY = infinity;
  for (; here < nedges && edge[here].topY <= interestingY; here++) {
    /* look at each new interesting edge */
    register int i, n;
  ...
  /* TMD - Compute all values before doing sort to save on math */
  while(i-- > 0) {
    topY = (*intp)->topY;
    bottomY = (*intp)->bottomY;
    topX = (*intp)->topX;
    bottomX = (*intp)->bottomX;
    if (topY > interestingY && topY < nextY)        nextY = topY;
    if (bottomY > interestingY && bottomY < nextY)  nextY = bottomY;

    (*intp)->value = Xvalue(topX, topY, bottomX, bottomY, interestingY + 1);
    intp++;
  }
  qsort ((char *) interesting, (unsigned) ninteresting,
	 sizeof (struct edge *), intercmp);
}
...
static int AddEdge (e, from, to, clip)
     struct edge *e;
     HardPoint from, to;
     int clip;
{
...
static void BuildEdgeList (path, clip)
     Path path;
     int clip;
{
  register Path p;
...
static int NextLowest (y)
     int y;
{
  int res;
  register struct lowest *p;
  
  for (p = lowest; p && p->e <= y; p = lowest) {
    /* delete any which are now irrelevent */
    lowest = p->higher;
