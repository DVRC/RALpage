...
InitPath ()
{
  ...
  InstallOp ("arc",		PArc,		5, 0, 0, 0, Float, Float,
	     Float, Float, Float);
  InstallOp ("arcn",		PArcN,		5, 0, 0, 0, Float, Float,
	     Float, Float, Float);
  InstallOp ("arcto",		PArcTo,		5, 4, 0, 0, Float, Float,
	     Float, Float, Float);
  InstallOp ("curveto",		PCurveTo,	6, 0, 0, 0, Float, Float,
	     Float, Float, Float, Float);
  InstallOp ("rcurveto",	PRCurveTo,	6, 0, 0, 0, Float,
  ...
  InstallOp ("pathforall",	PPathForAll,	4, 0, 0, 6, Array, Array,
	     Array, Array);
     ...
     Object x, y;
{
  ...
  if(vfp != NULL) {
    sprintf(foo, "PMoveto XY: %f %f", BodyReal(x), BodyReal(y));
    Message(foo);
  }

  x1 = BodyReal(x);    y1 = BodyReal(y);
  mp = &gstate->CTM;
  new.hx = x1 * mp->A + y1 * mp->C + mp->tx;
  new.hy = x1 * mp->B + y1 * mp->D + mp->ty;
  return GMoveTo (gstate->path, new);
}

static int PRMoveTo (x, y)
     Object x, y;
{
  register Matrix *mp;
  HardPoint new;
...
**  if (!gstate->cp_defined)
**    return Error (PNoCurrentPoint);
**  return GMoveTo (gstate->path, ExtToInt (MoveUserPoint
**       (NewUserPoint (BodyReal (x), BodyReal (y)), IntToExt (gstate->cp))));
*/
  if (!gstate->cp_defined) return Error (PNoCurrentPoint);

  x1 = BodyReal(x);  y1 = BodyReal(y);
  mp = &gstate->CTM;
  new.hx = x1 * mp->A + y1 * mp->C + gstate->cp.hx;
  new.hy = x1 * mp->B + y1 * mp->D + gstate->cp.hy;
  return GMoveTo (gstate->path, new);
}

static int PLineTo (x, y)
     Object x, y;
{
  register Matrix *mp;
  HardPoint new;
  ...
  new.hx = x1 * mp->A + y1 * mp->C + mp->tx;
  new.hy = x1 * mp->B + y1 * mp->D + mp->ty;
  return GLineTo (gstate->path, new);
}

static int PRLineTo (x, y)
     Object x, y;
{
  register Matrix *mp;
  HardPoint new;
  ...
  /*  if (!gstate->cp_defined)
  **    return Error (PNoCurrentPoint);
  ** return GLineTo (gstate->path, ExtToInt (MoveUserPoint
  **     (NewUserPoint (BodyReal (x), BodyReal (y)), IntToExt (gstate->cp))));
  */
...
static int PArc (x, y, rad, ang1, ang2)
     Object x, y, rad, ang1, ang2;
{
  UserPoint centre, start;
  float a1 = Rad (BodyReal (ang1)), a2 = Rad (BodyReal (ang2));
  float r = BodyReal (rad);
  
  start = NewUserPoint (cos (a1) * r, sin (a1) * r);
  centre = NewUserPoint (BodyReal (x), BodyReal (y));
  /* DCJ:
   * Compiler barfed on original line.  Don't blame it.
   */
  if (EmptyPath (gstate->path)) {
    if (!GMoveTo (gstate->path, ExtToInt (MoveUserPoint (centre, start))))
      return FALSE;
  } else {
    if (!GLineTo (gstate->path, ExtToInt (MoveUserPoint (centre, start))))
      return FALSE;
  }
	
  /* 	if (!(*(EmptyPath (gstate->path) ? GMoveTo : GLineTo)) (gstate->path, ExtToInt (MoveUserPoint (centre, start))))
   * return FALSE;
   */
  return Arc (gstate->path, 1, centre, r, a1, a2);
}

static int PArcN (x, y, rad, ang1, ang2)
     Object x, y, rad, ang1, ang2;
{
  UserPoint centre, start;
  float a1 = Rad (BodyReal (ang1)), a2 = Rad (BodyReal (ang2));
...
static int PArcTo (X1, Y1, X2, Y2, rad)
     Object X1, Y1, X2, Y2, rad;
{
  ...
  return ArcTo (gstate->path, gstate->cp,
		BodyReal (X1), BodyReal (Y1),
		BodyReal (X2), BodyReal (Y2), BodyReal (rad),
		&xt1, &yt1, &xt2,  &yt2) &&
...
static int PCurveTo (x0, y0, x1, y1, x2, y2)
     Object x0, y0, x1, y1, x2, y2;
{
  if (!gstate->cp_defined)
    return Error (PNoCurrentPoint);
  return CurveTo (gstate->path,
		  ExtToInt (NewUserPoint (BodyReal (x0), BodyReal (y0))),
		  ExtToInt (NewUserPoint (BodyReal (x1), BodyReal (y1))),
		  ExtToInt (NewUserPoint (BodyReal (x2), BodyReal (y2))));
}

static int PRCurveTo (x0, y0, x1, y1, x2, y2)
     Object x0, y0, x1, y1, x2, y2;
{
  UserPoint cp;
  ...
  cp = IntToExt (gstate->cp);
  return CurveTo (gstate->path,
		  ExtToInt (MoveUserPoint (cp,
					   NewUserPoint (BodyReal (x0),
							 BodyReal (y0)))),
		  ExtToInt (MoveUserPoint (cp,
					   NewUserPoint (BodyReal (x1),
							 BodyReal (y1)))),
		  ExtToInt (MoveUserPoint (cp,
					   NewUserPoint (BodyReal (x2),
							 BodyReal (y2)))));
}
...
static int PPathForAll (move, line, curve, close)
     Object move, line, curve, close;
{
  Path res;
  
  if ((res = PathCopy (gstate->path)) == NULL ||
      path_depth == PATHFORALLDEPTH - 1)
    return Error (PLimitCheck);
  *++pathp = res;
  ++path_depth;
...
static int PPathBBox ()
{
  UserPoint right_top, left_bottom, left_top, right_bottom;
