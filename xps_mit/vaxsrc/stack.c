...
static int PPop (), PDup (), PExch (), PRoll (), PIndex (), PClear ();
static int PCount (), PClearToMark (); 
...
static StackOb MakeStack (size, over, under)
     int size; Object over, under;
{
  StackOb res;
...
/*ARGSUSED*/
static int PPop (item)
     Object item;
{
  return TRUE;
}

static int PDup (item)
     Object item;
{
  return Push (OpStack, item), Push (OpStack, item);
}

static int PExch (a, b)
     Object a, b;
{
  return Push (OpStack, b), Push (OpStack, a);
}

static int PRoll (size, count)
     Object size, count;
{
