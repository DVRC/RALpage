...
InitImage ()
{
  InstallOp ("realimage",	RealImage,	5, 0, 0, 0,
	     Integer, Integer, Integer, Array, String);
  InstallOp ("realimagemask",	RealImageMask,	5, 0, 0, 0,
	     Integer, Integer, Bool, Array, String);
}

static int RealImageMask (width, height, invert, mat, string)
     Object width, height, invert, mat, string;
{
  Matrix m, m2;
  Vector origin, right, bottom;
  int w = BodyInteger (width), h = BodyInteger (height);
  int inv = BodyBool (invert), l = lengthString (string);
  int dwidth, dheight;
  ...
  dwidth = (int) (right.vx - origin.vx);
  dheight = (int) (bottom.vy - origin.vy);
  ...
  if (origin.vx == bottom.vx && origin.vy == right.vy &&
    ...
    if (dwidth != w || dheight != h) {
      to = ScaleBitmap (from, (float) dwidth / w,
			(float) dheight / h);
...
DrawBitmap (s, width, height, inv)
     unsigned char *s;
     int width, height, inv;
{
  int w, h;
     ...
     struct hardware *t, *h;
{
  ...
  BitBlt (t, h, NewDevicePoint (0, 0), NewDevicePoint (0, 0), xt,
	  ROP_SOURCE);
  
  sw = xt.dx;
  for (i = sw;;) {
    BitBlt (h, h, NewDevicePoint (0, 0), NewDevicePoint (i, 0),
	    NewDevicePoint (sw, xt.dy), ROP_SOURCE);
    if (i >= xh.dx)
      break;
    i += sw;
    sw <<= 1;
  ...
  sh = xt.dy;
  for (i = sh;;) {
    BitBlt (h, h, NewDevicePoint (0, 0), NewDevicePoint (0, i),
	    NewDevicePoint (xh.dx, sh), ROP_SOURCE);
    if (i >= xh.dy)
      break;
    i += sh;
    sh <<= 1;
...
struct hardware *UnpaddedHardwareFromString (s, w, h)
     unsigned char *s;
     int w, h;
{
  ...
  for (i = 0; i < h; i++)
    BitBlt (thin, res, NewDevicePoint (i * w, 0),
	    NewDevicePoint (0, i), NewDevicePoint (w, 1),
	    ROP_SOURCE);
...
static void VerticalReverseImage (h)
     struct hardware *h;
{
  DevicePoint ex;
  int i, n;
  ...
  for (i = 0; i < n / 2; i++)
    BitBlt (h, h, NewDevicePoint (0, i),
	    NewDevicePoint (0, n - i - 1),
	    NewDevicePoint (ex.dx, 1), ROP_XOR),
    BitBlt (h, h, NewDevicePoint (0, n - i - 1),
	    NewDevicePoint (0, i),
	    NewDevicePoint (ex.dx, 1), ROP_XOR),
    BitBlt (h, h, NewDevicePoint (0, i),
	    NewDevicePoint (0, n - i - 1),
...
static int RealImage (width, height, depth, mat, string)
     Object width, height, depth, mat, string;
{
  ...
  if (dheight < 0)
    origin.vy += (float) dheight, right.vy += (float)dheight,
    ++vrev, dheight = -dheight;
  ...
  if (dep != 16 && (int) origin.vx == (int) bottom.vx && (int) origin.vy == (int) right.vy &&
	...
	BitBlt (from, mask, NewDevicePoint (0, 0),
		NewDevicePoint (0, 0),
		HardwareExtent (from), ROP_NXOR);
	BitBlt (NULL, temp, NewDevicePoint (0, 0),
		NewDevicePoint (0, 0),
		HardwareExtent (temp), ROP_TRUE);
	for (j = 0; j < dep; j++)
	  BitBlt (mask, temp,
		  NewDevicePoint (j, 0),
		  NewDevicePoint (0, 0),
	...
	for (j = 0; j < w; j++)
	  BitBlt (temp, new,
		  NewDevicePoint (j * dep, 0),
		  NewDevicePoint (j, 0),
		  NewDevicePoint (1, h),
		  ROP_SOURCE);
	to = ScaleBitmap (new, (float) dwidth / w,
			  (float) dheight / h);
	Paint (to, gstate->device->dev,
	       NewDevicePoint (0, 0),
	       NewDevicePoint ((int) origin.vx,
    ...
    if (vrev)
      origin.vy += (float) dheight,
      right.vy += (float) dheight, dheight = -dheight;
...
DrawColourBitmap (s, width, height, depth)
     unsigned char *s;
     int width, height, depth;
{
  ...
  switch(depth) {
    case 16:
      ...
      for (h = 0; h < height; h++)
	for (w = 0; w < width; w++) {
	  gstate->colour = NewColour (0.0, 0.0,
	      (float) ((s[i] << 8) | s[i+1]) / (1 << 16));
	  FillUnitBox(w, h);
	  i += 2;
	}
      break;
    case 8:
      for (c = 0; c < 256; c++) {
	gstate->colour = NewColour (0.0, 0.0,(float) c/255);
	i = 0;
	for (h = 0; h < height; h++)
	  for (w = 0; w < width; w++)
	    if (c == s[i++]) FillUnitBox(w, h);
      }
      break;
    case 4:
      for (c = 0; c < 16; c++) {
	gstate->colour = NewColour (0.0, 0.0,(float) c/15);
	i = 0;
	for (h = 0; h < height; h++) {
	  for (w = 0; w < width; w++)
	    if(c == ((s[i+(w>>1)] >> ((1-(w&1)<<2))) & 15)) FillUnitBox(w,h);
	  i += width>>1;
	}
      }
      break;
    case 2:
      for (c = 0; c < 4; c++) {
	gstate->colour = NewColour (0.0, 0.0,(float) c/3);
	i = 0;
	for (h = 0; h < height; h++) {
	  for (w = 0; w < width; w++)
	    if (c == ((s[i+(w>>2)] >> ((3-(w&3))<<1)) & 3)) FillUnitBox(w,h);
	  i += width>>2;
	}
      }
      break;
    case 1:
      ...
      for (h = 0; h < height; h++) {
	for (w = 0; w < width; w++)
	  if (0 == ((s[i+(w>>3)] >> (7-(w&7))) & 1)) FillUnitBox(w,h);
	i += width>>3;
      }
      break;
  }
     ...
     int w, h;
{
  ...
  P.hx = x*a + y*c + m->tx;
  P.hy = x*b + y*d + m->ty;  VOID GMoveTo (gstate->path, P);
  P.hx += a;  P.hy += b;     VOID GLineTo (gstate->path, P);
  P.hx += c;  P.hy += d;     VOID GLineTo (gstate->path, P);
  P.hx -= a;  P.hy -= b;     VOID GLineTo (gstate->path, P);
  VOID ClosePath (gstate->path);
  VOID Fill ();
}

struct hardware *ScaleBitmap (from, xscale, yscale)
...
{	 	 	
  ...
  dwidth = (int) ((float) w * xscale + 0.5);
  dheight = (int) ((float) h * yscale + 0.5);
