     ...
     float a;
{
  ...
  Matrix r;
  NewMatrix(r, m.A * ca + m.C * sa, m.B * ca + m.D * sa,
...
{
  Matrix r;
  NewMatrix (r,    a.A  * b.A + a.B * b.C, a.A * b.B + a.B * b.D,
...
Matrix MatInvert (m)
     Matrix m;
     /* know any good matrix inversion algorithms ? */
{					/* this one will be simplistic */
  Matrix r;
  float detinv = 1 / (m.A * m.D - m.B * m.C);
  
  NewMatrix(r,m.D * detinv,
	    -m.B * detinv,
	    -m.C * detinv,
	    m.A * detinv,
	    (m.C * m.ty - m.D * m.tx) * detinv,
	    -(m.A * m.ty - m.B * m.tx) * detinv);
  return r;
}

Vector Transform (v, m)
     Vector v;
     Matrix m;
{
