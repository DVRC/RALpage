...
InitMatrix ()
{
  ...
  InstallOp ("concatmatrix",	PConcatMatrix,	3, 1, 0, 0, Array, Array,
	     Array);
     ...
     Object o;
{
  ...
  NewMatrix(*pm, temp[0], temp[1], temp[2], temp[3], temp[4], temp[5]);
  return TRUE;
}
...
static int PDefaultMatrix (array)
     Object array;
{
  if (lengthArray (array) != 6)
    return Error (PRangeCheck);
...
static int PCurrentMatrix (array)
     Object array;
{
  if (lengthArray (array) != 6)
    return Error (PRangeCheck);
...
static int PTranslate ()
{
  ...
  if (OpStack->stack_fill < 2) return Error(POpUnderflow);
  mat = Pop (OpStack);
  if (TypeOf (mat) == Array)
    if (!wCheck (mat))
      ...
      ty = Pop (OpStack);
      if (TypeOf (ty) == Integer)   y = (float) BodyInteger (ty);
      else if (TypeOf (ty) == Real) y = BodyReal (ty);
      else return Push2(OpStack, ty, mat), Error (PTypeCheck);

      tx = Pop (OpStack);
      if (TypeOf (tx) == Integer)   x = (float) BodyInteger (tx);
      else if (TypeOf (tx) == Real) x = BodyReal (tx);
      else return Push3(OpStack, tx, ty, mat), Error (PTypeCheck);
      
      NewMatrix(m, 1.0, 0.0, 0.0, 1.0, x, y);
      VOID AssignMatrix (mat, m);
      
      return Push (OpStack, mat);
    }
  else if (TypeOf (mat) == Integer) y = (float) BodyInteger (mat);
  else if (TypeOf (mat) == Real)    y = BodyReal (mat);
  else return Push (OpStack, mat), Error (PTypeCheck);
  
  tx = Pop (OpStack);
  if (TypeOf (tx) == Integer)   x = (float) BodyInteger (tx);
  else if (TypeOf (tx) == Real) x = BodyReal (tx);
  else return Push2(OpStack, tx, mat), Error (PTypeCheck);

  gstate->CTM.tx += x * gstate->CTM.A + y * gstate->CTM.C;
  gstate->CTM.ty += x * gstate->CTM.B + y * gstate->CTM.D;
  return TRUE;
}
...
static int PScale ()
{
  ...
  if (TypeOf (mat) == Array)
	...
	return Push2(OpStack, ty, mat), Error (PTypeCheck);
      tx = Pop (OpStack);
	...
	return Push3(OpStack, tx, ty, mat), Error (PTypeCheck);
      
      NewMatrix (m, x, 0.0, 0.0, y, 0.0, 0.0);
      VOID AssignMatrix (mat, m);
  ...
  if (TypeOf (tx) == Integer)
    ...
    return Push2(OpStack, tx, mat), Error (PTypeCheck);
...
static int PRotate ()
{
  ...
  if (OpStack->stack_fill < 1) return Error(POpUnderflow);
  else if(OpStack->stack_fill>OpStack->stack_size) return Error(POpOverflow);
  mat = Pop (OpStack);
  if (TypeOf (mat) == Array)
    if (!wCheck (mat))
      return Push (OpStack, mat), Error (PInvAccess);
	...
	return Push (OpStack, ang), Push (OpStack, mat), Error (PTypeCheck);
      ...
      ca = cos(a);  sa = sin(a);
      NewMatrix (m, ca, sa, -sa, -ca, 0.0, 0.0);
      VOID AssignMatrix (mat, m);
...
static int PConcatMatrix (mat1, mat2, mat3)
     Object mat1, mat2, mat3;
{
  Matrix m1, m2;
     ...
     Vector (*fn)();
{
  ...
  if (OpStack->stack_fill < 2) return Push(OpStack,mat), Error(POpUnderflow);
  ty = Pop (OpStack);
  if (TypeOf (ty) == Integer)
    y = (float) BodyInteger (ty);
  else if (TypeOf (ty) == Real)
...
static int PInvertMatrix (mat1, mat2)
     Object mat1, mat2;
{
  Matrix m;
     ...
     HardPoint P;
{
  ...
  /* TMD - Try really open coding this...
  **  	v = Transform (NewVector ((float) p.hx, (float) p.hy, 1.0),
  **                   MatInvert (gstate->CTM));
  **    return NewPoint (v.vx, v.vy);
  */
  mp = &gstate->CTM;
  det = mp->A * mp->D - mp->B * mp->C;
  x = P.hx - mp->tx;  y = P.hy - mp->ty;
  p.x = (x * mp->D - y * mp->C) / det;
  p.y = (y * mp->A - x * mp->B) / det;
  return p;
}

HardPoint ExtToInt (p)
     UserPoint p;
{
  HardPoint P;
  register Matrix *mp;
 ...
 */
  mp = &gstate->CTM;
  P.hx = p.x * mp->A + p.y * mp->C + mp->tx;
  P.hy = p.x * mp->B + p.y * mp->D + mp->ty;
  return P;
}

Matrix PointTranslate (m, p)
     Matrix m;
     UserPoint p;
{
